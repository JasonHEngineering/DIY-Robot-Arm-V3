...
def div_stepper(axis, step_data, selected_state):
    ...
    encoder_id__stepper_string = "encoder_stepper_" + str(axis+1)   
    ...
    body_layout = html.Div([
        ...
        html.B(id=encoder_id__stepper_string,style={'color': 'grey', 'fontSize': 16,'margin-right':5}),
...
def display_page(value):
    ...
    if (value == "Angles") or (value == "Steppers"):
        ...
        div_layout = html.Div([
            ...
            dcc.Interval(
                id="interval-1s",
                interval=2000,   # milliseconds (1 second)
                n_intervals=0
            ),
...
# auto-update encoder readings every 15 sec 
@app.callback(
    ...
    dash.dependencies.Output('target_coordinates', 'children'),
    [dash.dependencies.Input("interval-1s", "n_intervals"),
    ])
def update_encoder_every_1s(n):
...
def toggle(n, stored):
    ...
    color = "success" if new_state else "secondary"
...
def load(stored):
    if stored is None:
        return "Stepper ON", "secondary"
    return ("Stepper OFF", "success") if stored else ("Stepper ON", "secondary")
...
def toggle_PS_control(n, stored):
    ...
    if new_state:
        label = "ON"
        send_mode_PS_control("enable_PS_control")
    else: 
        label = "OFF"
        send_mode_PS_control("disable_PS_control")
...
def load_PS_control(stored):
    if stored is None:
        return "PS Controller ON", "secondary"
    return ("PS Controller OFF", "success") if stored else ("PS Controller ON", "secondary")
