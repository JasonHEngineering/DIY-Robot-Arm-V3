...
volatile long encoderSteps[no_of_steppers];
...
void taskCore0(void *parameter) {
  ...
  while (true) {
    // 1. Read encoders → convert to step units
    for (int i = 0; i < 6; i++) {
      // float angleDeg = readAbsoluteEncoder(i);
      read_angle_Register(); // Encoder read out should be encoder.angles_array[i]
      //long steps = angleDeg * ANGLE_TO_STEPS;

      xSemaphoreTake(feedbackMutex, portMAX_DELAY);
      //encoderSteps[i] = steps;
      xSemaphoreGive(feedbackMutex);
    }

    // 2. Receive new targets from Raspberry Pi (6× int16 → 12 bytes)
    // Wire.requestFrom(0x10, 12);
    // if (Wire.available() == 12) {
    //   xSemaphoreTake(feedbackMutex, portMAX_DELAY);
    //   for (int i = 0; i < 6; i++) {
    //     int16_t raw = Wire.read() | (Wire.read() << 8);
    //     targetSteps[i] = (long)raw;
    //   }
    //   xSemaphoreGive(feedbackMutex);
    // }

    // 3. Send encoder positions back to Pi (optional example packet)
    // xSemaphoreTake(feedbackMutex, portMAX_DELAY);
    // Wire.beginTransmission(0x10);
    // for (int i = 0; i < 6; i++) {
    //   int16_t out = (int16_t)encoderSteps[i];
    //   Wire.write(out & 0xFF);
    //   Wire.write(out >> 8);
    // }
...
void taskCore1(void *parameter) {
  ...
  while (true) {
    ...
    if (flag == true) {
      for (int x = 0; x <= (no_of_steppers-1); x++) {
        xSemaphoreTake(feedbackMutex, portMAX_DELAY);
        // long actual = encoderSteps[x];
        // long target = targetSteps[x];
        long actual = encoder.angles_array[x];
        long target = converter.valueReading[x];

        Serial.print("actual: ");
        Serial.println(encoder.angles_array[x]);
        Serial.print("target: ");
        Serial.println(converter.valueReading[x]);
      ...
      // motor.steps[x] = round(relative_target.angles_array[x]/(360.0/steps_per_rotation[x]) * gear_reduction[x] * micro_steps[x] );
        ...
        // if (abs(error_angle) < 2.0) {
        //   Error[x] = true; // true means this error amount is acceptable, in angle degree
        // }
        ...
        if (abs(error) < 10) {
          Error[x] = true; // true means this error amount is acceptable, in steps
        }
      ...
      // Run steppers smoothly
      for (int x = 0; x <= (no_of_steppers-1); x++) {
        steppers[x]->run();
      }

      // condition to set flag false again i.e. goal reached
      bool allTrue  = std::all_of(std::begin(Error), std::end(Error), [](bool v) {
          return v;
      });
...
void receiveData(int byteCount){
  ...
  switch (msgType) {
    ...
    case MSG_STEPPER_ARRAY: {
      ...
      // reset all axis flag to false
      for (int x = 0; x < no_of_steppers; x++) {
        Serial.print("Error[x] before: ");
        Serial.println(Error[x]);
        Error[x] = false;
      }

      for (int x = 0; x < no_of_steppers; x++) {
        Serial.print("Error[x] after: ");
        Serial.println(Error[x]);
      }
