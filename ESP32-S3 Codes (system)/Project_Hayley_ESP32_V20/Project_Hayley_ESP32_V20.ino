...
int steps_error_limit[no_of_steppers] = {10, 10, 10, 10, 10, 2}; // set microstep tolerance limit allow by PID that will mark the target as complete
...
while (true) {
    ...
    if (xQueueReceive(jointQueue, &converter.valueReading, 0) == pdTRUE) {
      ...
      if (isLastItem) {
          ...
          steps_error_limit[0] = 10; // tighten error at the last robotic step only
          steps_error_limit[1] = 10; // tighten error at the last robotic step only
          steps_error_limit[2] = 10; // tighten error at the last robotic step only         
          steps_error_limit[3] = 10; // tighten error at the last robotic step only
          steps_error_limit[4] = 10; // tighten error at the last robotic step only
          steps_error_limit[5] = 2; // tighten error at the last robotic step only   
      }
      else {
